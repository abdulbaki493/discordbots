...
const command = args.shift().toLowerCase();
  ...
  if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.channel.send('This command requires `MANAGE_MESSAGES` privilege');
      if(!sil) return message.channel.send('You need to specify an amount between 2-200');
...
const command = args.shift().toLowerCase();
  ...
  if (message.content.startsWith(prefix + 'hug')) {
      if (message.mentions.users.size < 1) return message.reply('please label the person you want to hug');
...
client.on("message", msg => {
  if (msg.content === prefix + "buysmallpet") {
    ...
    if (msg.guild){
        if (!pre[msg.guild.id]){
      msg.reply('Only premium servers and support server1')
    }
    if (pre[msg.guild.id]){
    let author = db.fetch(`reddia_${msg.author.id}`);
